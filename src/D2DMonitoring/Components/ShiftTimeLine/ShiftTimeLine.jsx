import React from 'react';
import styles from "../../Pages/D2DRealtime/Realtime.module.css";
import { AlertCircle, Check, Clock } from 'lucide-react';

const ShiftTimeLine = ({ events, activeConnectorIndex = -1, onEventClick }) => {
    return (
        <div className={styles.shiftTimelineCard}>
            <div className={styles.shiftTimelineTrack}>
                {events.map((event, index) => {
                    const isGray = event.time === "00:00" || event.time === "--:--" || event.isGray;
                    const isClickable = !!onEventClick &&
                        (event.key === "dutyOn" || event.key === "dutyOff") &&
                        event.time && event.time !== "00:00" && event.time !== "--:--";

                    const statusCap = event.status.charAt(0).toUpperCase() + event.status.slice(1);
                    const icon = event.status === "completed"
                        ? <Check size={10} />
                        : event.status === "active"
                            ? <Clock size={10} />
                            : <AlertCircle size={10} />;

                    return (
                        <React.Fragment key={event.key}>
                            <div
                                className={`${styles.shiftEvent} ${styles[`shiftEventChip_${event.status}`]} ${isClickable ? styles.shiftEventClickable : ""}`}
                                onClick={isClickable ? () => onEventClick(event) : undefined}
                                title={isClickable ? `Open ${event.label} details` : undefined}
                            >
                                {/* Icon */}
                                <div className={`${styles.shiftEventIconWrap} ${styles[`shiftEventIcon${statusCap}`]} ${isClickable ? styles.shiftEventIconClickable : ""} ${isGray ? styles.grayTimeIcon : ""}`}>
                                    {icon}
                                    {isClickable && <span className={styles.shiftEventRipple} />}
                                </div>

                                {/* Label + Time stacked */}
                                <div className={styles.shiftEventMeta}>
                                    <span className={`${styles.shiftEventLabel} ${event.status === "active" ? styles.shiftEventLabelActive : ""} ${isGray ? styles.grayTime : ""}`}>
                                        {event.label}
                                    </span>
                                    <span className={`${styles.shiftEventTime} ${event.status === "pending" ? styles.shiftEventTimePending : ""} ${event.isLive ? styles.shiftEventTimeLive : ""} ${isGray ? styles.grayTime : ""}`}>
                                        {event.isLive && <span className={styles.liveDot} />}
                                        {event.time}
                                        {isClickable && <span className={styles.shiftEventArrow}>›</span>}
                                    </span>
                                </div>
                            </div>

                            {index < events.length - 1 && (
                                <div className={`${styles.shiftEventConnector} ${index < activeConnectorIndex ? styles.shiftEventConnectorCompleted : index === activeConnectorIndex ? styles.shiftEventConnectorActive : styles.shiftEventConnectorPending}`} />
                            )}
                        </React.Fragment>
                    );
                })}
            </div>
        </div>
    );
};

export default ShiftTimeLine;
